...
verbose = 1; % verbose = 0: no visual output, 1: only visual output in the end, 2: all visual output
%verbose = 0; % verbose = 0: no visual output, 1: estimates and/or odometry and/or groundtruth, 2: (1)+ extra info, 3: (2)+ observation lines

ITERATIONS = 50; % 20 default
%CONVERGE = 1e-05 % default
CONVERGE = 1e-03;
%CONVERGE = 1;
...
simLen = length(simSteps);

for i=2:(simLen-1)
    
  prevL = simSteps{i-1}.seenLandmarks;
  currL = simSteps{i}.seenLandmarks;
  nextL = simSteps{i+1}.seenLandmarks;  
  
  prevX = simSteps{i-1}.seenLandmarks(1:2);
  currX = simSteps{i}.seenLandmarks(1:2);
  nextX = simSteps{i+1}.seenLandmarks(1:2);
  
  prevNoSeenLandmarks = size(prevX);
  currNoSeenLandmarks = size(currX);
  nextNoSeenLandmarks = size(nextX);
  
  if (currNoSeenLandmarks(2) == prevNoSeenLandmarks(2)) && (currNoSeenLandmarks(2) == nextNoSeenLandmarks(2))
      
      for landmark=1:currNoSeenLandmarks
          dist1 = norm(currX - prevX, 2);
          dist2 = norm(currX - nextX, 2);

          if dist1 > DISTLIM && dist2 > DISTLIM
              newX = [];
              %Outlier!
              %fprintf('Detected outlier! i=%f',i);
              %fprintf('\n');
  
              %fprintf('Previously \n');
              %simSteps{i}.seenLandmarks
              newX = (prevX + nextX)/2;
              if currNoSeenLandmarks > 1
                  simSteps{i}.seenLandmarks(1:2,landmark) = newX;
                  %fprintf('Updated to \n');
                  %simSteps{i}.seenLandmarks
              else
                  simSteps{i}.seenLandmarks(1:2) = newX;
                  %fprintf('Updated to \n');
                  %simSteps{i}.seenLandmarks
              end
          end
...
temp=round(numPoses/partitions,0);
finish=[1,temp*[1:partitions-1], numPoses];
